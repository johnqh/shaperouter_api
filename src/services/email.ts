...
const senderName = getEnv("RESEND_SENDER_NAME", "ShapeRouter");
...
export async function sendInvitationEmail({
  ...
  const { error } = await resend.emails.send({
    ...
    to: recipientEmail,
    subject: `You've been invited to join ${entityName} on ShapeRouter`,
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 560px; margin: 0 auto; padding: 40px 20px;">
        <h2 style="color: #111; margin-bottom: 16px;">You're invited!</h2>
        <p style="color: #333; font-size: 16px; line-height: 1.5;">
          You've been invited to join <strong>${entityName}</strong> on ShapeRouter.
        </p>
        <p style="color: #333; font-size: 16px; line-height: 1.5;">
          Sign in to your ShapeRouter account to accept the invitation:
        </p>
        <a href="${dashboardUrl}"
